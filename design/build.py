...
#
# Канон объявляет о себе проверяемые вещи: «полки над базой обесцвечены»,
# «тон полок стоит в 285.7±0.1», «янтарь всего в 9° от sem.warning»,
# «ультрамарин даёт 4.0:1 и не проходит». Заявление, которое некому проверить, — не
...
def ramp_block(tokens, group_name, names, title, hint, prove_steps, block_id=None):
    ...
    ней стоит столбик длиной ∝ ΔL: неравенство шага (от базы до первой полки
    он вдвое больше прочих) доска ОБЯЗАНА показать глазу, а не пересказать
    словами — иначе «намеренно» и «недосмотр» выглядят одинаково.
    """
    steps = []
    previous = None
    for name in names:
    ...
    if prove_steps:
        # Столбик длиной ∝ ΔL, посаженный на стык ступеней. Длина — ДОЛЯ
        # колонки, а не абсолютная величина: множитель в px подбирался под
        # шаг 0.03, и на шаге 0.082 самый длинный столбик перерос свою
        # колонку. Колонка на `1fr` имеет min-width: auto, поэтому она
        # раздалась под него — и сетка столбиков разъехалась с полосой над
        # ней, то есть столбик перестал показывать на стык, который меряет.
        # Доля от самого длинного шага держит и пропорцию, и стык, и не
        # знает про единицы длины вовсе. minmax(0,1fr) чинит ту же ловушку
        # на будущее: колонка больше не растягивается под содержимое.
        widest = max(abs(step["delta"]) for step in steps if step["delta"] is not None)
        lines.append('  <div class="ramp-deltas" style="grid-template-columns: %s">'
                     % columns.replace("1fr", "minmax(0, 1fr)"))
        for index, step in enumerate(steps):
            if step["delta"] is None:
                continue
            width = "%.1f%%" % (abs(step["delta"]) / widest * 100.0)
...
def colors_blocks(tokens):
    ...
    lines.extend(ramp_block(
        ...
        "и от базы до первой полки шаг вдвое больше прочих", True))
...
SCREEN_GROUPS = [
    {
        "anchor": "screen-setup", "title": "Мастер настройки", "kind": "iframe",
        "src": "candidates/setup.html", "height": 14,
...
def board_screens(tokens):
    ...
    поверхности в нём не различить. Теперь — подкатегории (мастер настройки, главный,
