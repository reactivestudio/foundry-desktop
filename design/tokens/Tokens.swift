...
enum Token {
    ...
    /// Elevation через светлоту, не через тень: равные шаги ΔL ≈ 0.03 в OKLCH, hue удержан в фиолетовой зоне 290–295, хрома растёт с высотой. Слой не перепрыгивает уровни; уровней на экране ≤3. Сайдбар и тулбар — системный Liquid Glass, их фон не хардкодим.
    enum Background {
        /// фон окна, фон контентной зоны
        static let base = Color(hexValue: 0x07060B)
        /// карточки канбана, строки-контейнеры, панель кода/диффа/лога
        static let surface = Color(hexValue: 0x0D0C14)
        /// поповеры, меню, тултипы, sticky-заголовки
        static let raised = Color(hexValue: 0x14121E)
        /// модалки/шиты (плюс скрим под ними)
        static let overlay = Color(hexValue: 0x1B1828)
    ...
    /// Альфа-белый, не серые хардкоды. Ни один цвет палитры не используется как цвет текста: brand.ultramarine на тёмном даёт 3.9:1 и не проходит — текстом всегда text.accent. Контрасты посчитаны в 06-color.md §4.2 и указаны здесь на bg.base.
    enum Text {
        ...
        /// текст на ультрамарине (5.1:1) и пурпуре (4.2:1); на циане и мадженте белый ЗАПРЕЩЁН — там тёмный #07060B
