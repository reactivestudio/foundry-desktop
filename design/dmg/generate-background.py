...
Устройство фона — утверждённый эскиз (v26, 2026-07-17):
  ...
  · стрелка — росчерк одним ходом кривизны (см. _gesture): пологая горка,
    клотоида, окружность Ø52pt, и обратно; точки Ø2.1pt посажены по закону
    двух третей (_beads), наконечник по касательной; под ней тёмное гало;
...
def _gesture(R, c, Lp, Lr, phi):
    ...
    Нулевой кривизны нет нигде, кроме одной точки на стыке подхвата с клотоидой,
    ...
    при −c впритык к заходу от круга остаётся 254° и низ раздут; так — 284°).

    Полный поворот берём 2π−2φ. Профиль симметричен, поэтому кривая зеркальна
    относительно серединного перпендикуляра хорды: развернув хорду в горизонт,
    получаем вход под +φ и выход под −φ. Горка возникает сама из хода кривизны,
    её не подпирают ни прогибом, ни наклоном.
    """
    La = R * (2 * math.pi - 2 * phi + 2 * c * Lp / 3 - Lr / R)
    if La <= 0:
    ...
    a, b = Lp, Lp + Lr
    d, e = b + La, b + La + Lr
    k = np.where(arc < a, -c * (1 - arc / Lp) ** 2,
        np.where(arc < b, (arc - a) / Lr / R,
        np.where(arc < d, 1 / R,
        np.where(arc < e, (1 - (arc - d) / Lr) / R, -c * ((arc - e) / Lp) ** 2))))
...
def _path(x0, x1, y0, R=26.0, c=1 / 120, phi=math.radians(15), lead=0.18):
    """Хорда росчерка кладётся ровно в пролёт между иконками. Подгоняется не
    масштабом (он увёл бы радиус петли), а длиной подхвата Lp: хорда растёт по
    Lp монотонно, поэтому деление пополам. Дальше — поворот хорды в горизонт.

    lead — доля длины окружности на каждый заход-клотоиду. Она разменивает угол
    перекрестия на долю настоящей окружности в петле: 0.18 → 22° и 284° дуги,
    0.22 → 25° и 269°. При 0.18 ветви на перекрестии уже не идут бок о бок, а на
    круг остаётся 79% петли.
    """
    Lr = lead * 2 * math.pi * R
    lo, hi = 1.0, 400.0
    for _ in range(60):
        mid = (lo + hi) / 2
        gx, gy, _ = _gesture(R, c, mid, Lr, phi)
        if math.hypot(gx[-1] - gx[0], gy[-1] - gy[0]) < x1 - x0:
            lo = mid
        else:
            hi = mid
    gx, gy, k = _gesture(R, c, (lo + hi) / 2, Lr, phi)
...
def arrow_alpha(s, dot=2.1, spacing=6.5, ease=0.03):
    ...
    at = at[at <= acc[-1] - head * 0.92 - dot]   # бусины не заезжают под наконечник
    dx, dy = _uncollide(np.interp(at, acc, xs), np.interp(at, acc, ys), 4.3)
    lay = Image.new('L', (ws * K, hs * K), 0)
    d = ImageDraw.Draw(lay)
    M = s * K
    r = dot / 2 * M
    for x, y in zip(dx * M, dy * M):
