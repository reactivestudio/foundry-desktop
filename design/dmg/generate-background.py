...
Устройство фона — утверждённый эскиз (v26, 2026-07-17):
  ...
  · стрелка — точки Ø2.1pt, петля-окружность Ø52pt с клотоидными заходами
    (см. _loop), наконечник по касательной; под ней тёмное гало;
...
def _loop(R, frac, ds=0.05):
    """Петля задаётся ходом кривизны по длине дуги: 0 → 1/R (клотоида) →
    1/R (дуга окружности) → 0. Клотоида — то, как поворачивает рука: радиус
    ужимается постепенно. Скачок кривизны прямая→окружность читается изломом
    даже при совпадении касательных, а «остановка» горизонтального хода ради
    круглой петли — механикой.

    Суммарный поворот ровно 2π, поэтому выход параллелен входу, а профиль
    симметричен — вертикальный снос строго нулевой. Горизонтальный снос не
    задаётся, а получается из геометрии: петля сама съезжает вправо, и внизу
    возникает перекрестие. frac — доля длины на каждый переход; остальное
    достаётся круговой дуге (при 0.25 — 60% длины петли).
    """
    Lt = frac * 2 * math.pi * R
    L3 = R * (2 * math.pi - Lt / R)         # дуга: добор поворота до 2π
    if L3 <= 0:
        raise ValueError('переход длиннее окружности — на дугу ничего не остаётся')
    L = 2 * Lt + L3
    s = np.linspace(0, L, int(L / ds))
    k = np.where(s < Lt, s / Lt / R,
                 np.where(s < Lt + L3, 1 / R, np.clip((L - s) / Lt, 0, None) / R))
    th = np.concatenate([[0], np.cumsum((k[1:] + k[:-1]) / 2 * np.diff(s))])
    lx = np.concatenate([[0], np.cumsum((np.cos(th[1:]) + np.cos(th[:-1])) / 2 * np.diff(s))])
    ly = np.concatenate([[0], np.cumsum((-np.sin(th[1:]) - np.sin(th[:-1])) / 2 * np.diff(s))])
    return lx, ly


def _path(x0, x1, y0, R, frac, bow=4.0):
    """Прямая — петля — прямая, всё с едва заметным прогибом (bow). Прогиб
    берётся по x: у синуса в середине пролёта производная нулевая, поэтому
    петлю он сдвигает целиком, а не перекашивает."""
    lx, ly = _loop(R, frac)
    xA = (x0 + x1) / 2 - lx[-1] / 2         # петля по центру пролёта
    xs = np.concatenate([np.linspace(x0, xA, 400), xA + lx,
                         np.linspace(xA + lx[-1], x1, 400)])
    ys = np.concatenate([np.full(400, y0), y0 + ly, np.full(400, y0)])
    ys -= bow * np.sin(np.pi * np.clip((xs - x0) / (x1 - x0), 0, 1))
    return list(zip(xs, ys))


def arrow_alpha(s, R=26.0, frac=0.25, dot=2.1, spacing=6.5):
    """Бусины и наконечник, 4x суперсэмпл; гало к ним считается отдельно."""
    K = 4
    ws, hs = W * s, H * s
    y0 = ICONY + ICON // 2
    x0, x1 = APPX + ICON // 2 + 16, DSTX - ICON // 2 - 16
    pts = _path(x0, x1, y0, R, frac)
    ...
    while pos <= stop:
        ...
        pos += spacing
    ex, ey = pts[-1]
    back = next(i for i in range(len(acc)-1, -1, -1) if acc[-1] - acc[i] >= 6)
    px_, py_ = pts[back]           # касательная по хорде в 6pt: шаг точек пути
    ang = math.atan2(ey - py_, ex - px_)   # неравномерен, индексом её не мерить
    tx, ty = ex * M, ey * M
