...
Устройство фона — утверждённый эскиз (v26, 2026-07-17):
  ...
  · стрелка — росчерк одним ходом кривизны (см. _gesture): подхват, петля
    овалом с наклоном вперёд (плато нет — оно и есть циркуль), сбег; точки
    посажены по закону двух третей (_beads), калибр растёт Ø1.5→2.6pt;
...
def _gesture(Rmin, c, Lpi, Lpo, m, q, phi):
...
def _path(x0, x1, y0, Rmin=20.5, c=1 / 120, phi=math.radians(15),
    ...
    for _ in range(50):
        mid = (lo + hi) / 2
        gx, gy, _ = _gesture(Rmin, c, mid * (1 - skew), mid * (1 + skew), m, q, phi)
        if math.hypot(gx[-1] - gx[0], gy[-1] - gy[0]) < x1 - x0:
            lo = mid
        else:
            hi = mid
    mid = (lo + hi) / 2
    gx, gy, k = _gesture(Rmin, c, mid * (1 - skew), mid * (1 + skew), m, q, phi)
