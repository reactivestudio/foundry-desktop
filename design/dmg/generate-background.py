...
Устройство фона — утверждённый эскиз (v26, 2026-07-17):
  ...
  · стрелка — росчерк одним ходом кривизны (см. _gesture): подхват, клотоида,
    окружность Ø46pt с наклоном вперёд, и обратно; точки посажены по закону
    двух третей (_beads), калибр растёт Ø1.5→2.6pt; под ней тёмное гало;
...
def _gesture(R, c, Lpi, Lpo, Lri, Lro, phi):
    """Росчерк задан одним непрерывным ходом кривизны по длине дуги:

        −c·(1−u)²  подхват    u = arc/Lp, круче всего у самого хвоста
        0 → 1/R    клотоида   кривизна линейна по длине — так поворачивает рука
        1/R        окружность
        1/R → 0    клотоида   (своей длины: Lro ≠ Lri — этим петля наклонена)
        −c·u²      сбег

    Нулевой кривизны нет нигде, кроме двух точек на стыках подхвата с клотоидой,
    и в этом суть: прямая по линейке — первая примета машинного росчерка, рука
    так не ведёт. Скачок кривизны читался бы изломом даже при совпадении
    касательных, поэтому все переходы непрерывны.

    Ключ к тому, чтобы петля осталась КРУГОМ: обратная кривизна живёт только у
    концов и к петле сходит на нет, а в клотоиду заход входит уже с нуля. Если
    вести обратную кривизну до самой петли, её низ выдувает в каплю (проверено:
    при −c впритык к заходу от круга остаётся 254° и низ раздут; так — 283°).

    Полный поворот берём 2π−2φ, поэтому выход идёт на 2φ ниже входа. Ровно
    зеркальный профиль дал бы петлю строго вертикальной — она статична; наклон
    вперёд берётся из неравенства Lri ≠ Lro и Lpi ≠ Lpo.
    """
    La = R * (2 * math.pi - 2 * phi + c * (Lpi + Lpo) / 3 - (Lri + Lro) / (2 * R))
    if La <= 0:
        raise ValueError('заходы съели поворот — на окружность не осталось')
    L = Lpi + Lri + La + Lro + Lpo
    arc = np.arange(0, L, 0.04)
    a, b = Lpi, Lpi + Lri
    d, e = b + La, b + La + Lro
    k = np.where(arc < a, -c * (1 - arc / Lpi) ** 2,
        np.where(arc < b, (arc - a) / Lri / R,
        np.where(arc < d, 1 / R,
        np.where(arc < e, (1 - (arc - d) / Lro) / R, -c * ((arc - e) / Lpo) ** 2))))
    tz = lambda f: np.concatenate(
        [[0], np.cumsum((f[1:] + f[:-1]) / 2 * np.diff(arc))])
    th = tz(k)
    return tz(np.cos(th)), tz(-np.sin(th)), k


def _path(x0, x1, y0, R=23.0, c=1 / 120, phi=math.radians(15), lead=0.18,
          skew=0.28, lean=-0.30):
    """Хорда росчерка кладётся ровно в пролёт между иконками. Подгоняется не
    масштабом (он увёл бы радиус петли), а длиной подхвата: хорда растёт по ней
    монотонно, поэтому деление пополам. Дальше — поворот хорды в горизонт.

    lead — доля длины окружности на заход-клотоиду. Она разменивает угол
    перекрестия на долю настоящей окружности в петле: 0.18 → 22° и 284° дуги,
    0.22 → 25° и 269°. Ниже ~0.15 ветви на перекрестии идут бок о бок и петля
    читается баллоном на длинной шее.

    skew и lean — два РАЗНЫХ рычага, их легко перепутать и свалить в один:
    skew двигает петлю по пролёту (сбег длиннее подхвата — петля левее), lean
    её наклоняет. При lean = −0.30 заход-клотоида длиннее (33.8pt) и мягче, а
    выход короче (18.2pt) и резче — петля наклоняется вперёд, как в рукописи.
    Положительный lean заваливает её назад: обратный наклон, чужая рука.
    """
    Lri, Lro = lead * (1 - lean) * 2 * math.pi * R, lead * (1 + lean) * 2 * math.pi * R
    lo, hi = 1.0, 400.0
    for _ in range(60):
        mid = (lo + hi) / 2
        gx, gy, _ = _gesture(R, c, mid * (1 - skew), mid * (1 + skew), Lri, Lro, phi)
        if math.hypot(gx[-1] - gx[0], gy[-1] - gy[0]) < x1 - x0:
            lo = mid
        else:
            hi = mid
    mid = (lo + hi) / 2
    gx, gy, k = _gesture(R, c, mid * (1 - skew), mid * (1 + skew), Lri, Lro, phi)
    ang = -math.atan2(gy[-1] - gy[0], gx[-1] - gx[0])
    ca, sa = math.cos(ang), math.sin(ang)
    return x0 + gx * ca - gy * sa, y0 + gx * sa + gy * ca, k
