...
struct FoundryApp: App {
    ...
    // `FoundryApplication` поднимает контекст через `SparkApplication.run` (наш `SpringApplication.run`),
    ...
    var body: some Scene {
        ...
        // окно мастера настройки выходит выше макета (720×880 — это ПОЛНЫЙ размер с 44px
