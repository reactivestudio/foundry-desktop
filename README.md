...
Несущие правила: правда — в файлах `.foundry/` внутри проекта (папка
локальная, в `.gitignore` — не под git); мутации состояния — только через
`foundry` CLI; локальная БД (SQLite) — проекция + app-only данные (треды
ревью, снапшоты артефактов для ревью, настройки), пересобираемая, никогда не
источник правды.
...
- **Хранение — три места по роли** (2026-07-21). `.foundry/` внутри проекта
  (локальная, в `.gitignore`) — единственный источник правды, владелец — CLI;
  приложение туда своё не пишет. Централизованно, одно место на все проекты
  (привязка по пути к проекту) — история ранов и снапшоты артефактов для
  ревью: проекция, пересобираемая, не источник правды. Настройки приложения и
  список проектов — в Application Support. Сессии Claude не дублируем —
  читаем из `~/.claude/projects/`. Диффы: код — git (ветка рабочей копии ↔
  база), артефакты — снапшот прошлого ревью ↔ текущий файл (в git артефактов
  нет).
...
Workflow удаляет с релиза только собственные дистрибутивы — строго
`Foundry-X.Y.Z.dmg|zip` (и `FoundryDesktop-X.Y.Z.*` — имена до переименования).
Версия обязана быть ровно `X.Y.Z`: тег вида `v0.2.0-beta` workflow отклонит, а не
соберёт дистрибутив, который уборка потом не узнает. Всё прикреплённое руками
остаётся нетронутым, включая похожие имена: `Foundry-0.1.2-dSYMs.zip` или
`Foundry-nightly.zip` под уборку не попадают.
