...
```
foundry-desktop/
├── FoundryDesktop.xcodeproj        # тонкий: app-таргет + UITests, buildable folder → App/
├── App/                            # buildable folder
│   ├── FoundryDesktopApp.swift     # @main, wiring: собирает зависимости, отдаёт в FoundryKit
│   ├── FoundryDesktop.entitlements
│   ├── Assets.xcassets
...
- Smoke-запуск собранного приложения:
  `open .build/DerivedData/Build/Products/Debug/FoundryDesktop.app`
...
1. **`Package.resolved` коммитится** — и пакетный
   ...
   (`FoundryDesktop.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved`).
...
Стартовый `FoundryDesktop.entitlements` — **пустой словарь**. Каждый добавленный
...
xcodebuild archive \
  ...
  -archivePath .build/FoundryDesktop.xcarchive
...
```bash
ditto -c -k --keepParent .build/export/FoundryDesktop.app .build/FoundryDesktop.zip

xcrun notarytool submit .build/FoundryDesktop.zip \
  --keychain-profile "notary" --wait
# профиль один раз: xcrun notarytool store-credentials notary --key AuthKey.p8 --key-id … --issuer …

xcrun stapler staple .build/export/FoundryDesktop.app   # офлайн-валидация Gatekeeper

# Верификация перед публикацией
codesign --verify --deep --strict --verbose=2 .build/export/FoundryDesktop.app
spctl -a -vv .build/export/FoundryDesktop.app
```
...
```bash
npx create-dmg .build/export/FoundryDesktop.app .build/   # sindresorhus/create-dmg 8.x: zero-config, подписывает DMG
xcrun notarytool submit .build/FoundryDesktop*.dmg --keychain-profile notary --wait
xcrun stapler staple .build/FoundryDesktop*.dmg           # staple к самому DMG
```

Итого артефактов на релиз два: `FoundryDesktop.zip` (Sparkle) + `FoundryDesktop.dmg`
...
release version:
    just test
    xcodebuild archive -project FoundryDesktop.xcodeproj -scheme FoundryDesktop \
      -destination 'platform=macOS' -archivePath .build/FoundryDesktop.xcarchive \
      MARKETING_VERSION={{version}} CURRENT_PROJECT_VERSION=$(git rev-list --count HEAD)
    xcodebuild -exportArchive -archivePath .build/FoundryDesktop.xcarchive \
      -exportOptionsPlist scripts/ExportOptions.plist -exportPath .build/export
    ditto -c -k --keepParent .build/export/FoundryDesktop.app .build/FoundryDesktop.zip
    xcrun notarytool submit .build/FoundryDesktop.zip --keychain-profile notary --wait
    xcrun stapler staple .build/export/FoundryDesktop.app
    npx create-dmg .build/export/FoundryDesktop.app .build/
    ./scripts/sparkle/generate_appcast .build/updates
    gh release create v{{version}} .build/FoundryDesktop.zip .build/*.dmg \
