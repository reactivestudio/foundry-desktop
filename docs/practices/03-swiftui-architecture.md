...
сборка против macOS 26 SDK, deployment target — macOS 15 Sequoia**.
Почти всё в главе доступно с macOS 14–15; **26-only** API (`Observations`,
SwiftUI `WebView`, `.reorderable()`) помечены по месту — использовать только
за `#available` либо брать указанный fallback. Язык и конкурентность — в
...
> **26-only.** Рантайм `Observations` — macOS 26+; на нашем таргете
> (macOS 15) — fallback: `AsyncStream` из сервиса-владельца либо цикл на
> `withObservationTracking` (macOS 14+, тот же механизм, что под капотом).
...
«фильтр доски изменился → перезапросить проекцию». На таргете macOS 15 —
fallback из §1: `AsyncStream` или `withObservationTracking`-цикл.
...
>
> **Но:** всё в этом списке с пометкой macOS 26/WWDC26 — вне нашего таргета
> (macOS 15): web-контент по-прежнему через `WKWebView`-representable
> ([06](06-system-integration.md)), reorder/swipe — руками или AppKit.
...
| 15 | Combine/`objectWillChange` для связки store→сервис | AsyncStream / `withObservationTracking` (на 26+ — `Observations`) |
