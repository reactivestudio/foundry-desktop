...
- **Писать свой макрос** — только когда extension/protocol/generic не выражают
  задачу, а boilerplate реален (≥3 дублированных нетривиальных места). В проекте
  такой макрос один, в двух формах (таргет `CoreMacros`): `@Invariants` на корне
  агрегата расставляет проверку инвариантов по всем его командам, `@Invariant` на
  команде — точечно и с собственным методом проверки. Body-макрос дописывает в тело
  `defer { checkInvariants() }` — местный аналог AOP, только на компиляции, а не
  рантайм-перехватом, как в Spring. Грабли на этом пути: ссылку на метод
  (`Тип.метод`) в аргументе КЛАССОВОЙ аннотации не написать — «circular reference
  resolving attached macro», потому что разрешение ссылки требует раскрыть тот же
  макрос; поэтому там имя строкой, а типизированную ссылку макрос собирает уже
  внутри типа. И второе: после правки плагина пересборка бывает неполной — тесты
  гоняются на старых раскрытиях, лечится изменением файла-клиента.
- ⚠️ Устарело: «макросы убивают время сборки из-за swift-syntax» — Swift 6.2+
  ...
  худший clean-build-штраф ушёл. Оговорки, померенные на своём макро-таргете:
  prebuilt подхватывается только при совместимом пине (601.x — чистая сборка
  пакета 7 с; 603.x — swift-syntax собирается из исходников, 38 с) и только в
  `swift build`: Xcode свою сборку приложения всё равно ведёт из исходников
  swift-syntax. Наследовать `SyntaxVisitor` при этом нельзя — плагин не
  слинкуется с prebuilt-бинарём, обход дерева пишется рекурсией по `children`.
