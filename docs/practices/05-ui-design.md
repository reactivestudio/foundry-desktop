...
Таргет: сборка против **macOS 26 SDK (Xcode 26)**, **deployment target — macOS 15 Sequoia**.

> **Рамка (решение 2026-07-14): Liquid Glass — не цель.** Машина
> разработчика на Sequoia, апгрейд не планируется. §1 хранится как справка
> на будущее — при переезде на macOS 26+ он снова становится нормативным;
> его 26-only API (`glassEffect`, `backgroundExtensionEffect()`,
> `ScrollEdgeEffectStyle`) в код не тащить, «⚠️ Устарело» внутри §1 читать
> в этой рамке. Действующий дизайн-язык — Sequoia: системные материалы и
> vibrancy, стандартный `NavigationSplitView`-каркас, unified-тулбар —
> те же стандартные компоненты, просто в сегодняшнем системном виде.
> Остальные разделы главы (типографика, плотность, цвет, SF Symbols,
> паттерны экранов §6) от версии не зависят и действуют как есть.
