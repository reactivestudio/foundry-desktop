...
1. ~~Хранение истории ранов в приложении (SQLite)~~ — решено: централизованная
   проекция (история + снапшоты), пересобираемая; источник правды остаётся в
   `.foundry/`. Раскладка «три места по роли» — в разделе «Принятые решения»
   [README](../README.md).
