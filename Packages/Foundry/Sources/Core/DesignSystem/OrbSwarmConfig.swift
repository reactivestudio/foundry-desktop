...
public struct OrbSwarmConfig: Equatable, Sendable {
    ...
    public static let maxSupersample: Int = 8
    ...
    public enum Loader: String, CaseIterable, Sendable {
        ...
        var pointSizeOnScreen: Float { 1.0 }
    }
    ...
    /// Наименьшая степень двойки `ss` в [1, cap], при которой `perStep · ss`
    /// дотягивает до порога `target`. Единый выбор сверхвыборки для обоих роёв
    /// (логотип по порогу гладкости/рисования, мастер настройки — так же): алгоритм один,
    /// разнятся только `perStep` и `target`. Цикл множит от 1, порядок float-операций
    /// тот же, что был в развёрнутых копиях — раскладка бит-в-бит прежняя.
    public static func supersamplingFactor(perStep: Float, target: Float, cap: Int = maxSupersample) -> Int {
        var ss = 1
        while ss < cap, perStep * Float(ss) < target { ss *= 2 }
        return ss
    }
    ...
    public init(loader: Loader, scale: Float) {
        self.init(
            ...
            pointSizeOnScreen: loader.pointSizeOnScreen, size: loader.size, scale: scale)
    }

    /// Развязанная раскладка: `particlesPerPixel` задаёт СКОЛЬКО частиц,
    /// `pointSizeOnScreen` — какой они КРУПНОСТИ, порознь. Зерно тут производное —
    /// оно следствие крупности, а не наоборот, как у логотипа.
    public init(particlesPerPixel: Float, pointSizeOnScreen: Float, size: Float, scale: Float) {
        self.size = size
        self.scale = scale

        let out = max(1, Int((size * scale).rounded()))
        self.outputSide = out
        self.particleCount = max(1, Int((particlesPerPixel * Float(out * out)).rounded()))

        // Сведение — под гладкость мелкой точки, а не под порог рисования.
        let ss = Self.supersamplingFactor(
            perStep: pointSizeOnScreen * scale, target: Self.glossPointSize)
        self.supersample = ss
        self.bufferSide = out * ss

        let pt = pointSizeOnScreen * scale * Float(ss)  // экранные px → px буфера
        self.pointSize = pt
        // Крупность как доля тела — нужна порогу fps и как справка.
        self.grain = pt / (Self.orbBodyFraction * Self.zoom * Float(bufferSide))
    ...
    public init(grain: Float, size: Float, scale: Float) {
        ...
        let ss = Self.supersamplingFactor(
            perStep: grain * Self.orbBodyFraction * Self.zoom * Float(out), target: Self.minPointSize)
        self.supersample = ss
        self.bufferSide = out * ss

        let pt = grain * Self.orbBodyFraction * Self.zoom * Float(bufferSide)
        self.pointSize = pt
        self.particleCount = Int((Self.coverage / (grain * grain)).rounded())
        self.flickers = pt < Self.minPointSize
    }
    ...
    public var particlesPerPixel: Float { Float(particleCount) / Float(outputSide * outputSide) }
    ...
    public static func stutters(preset: Preset, displayHz: Int) -> Bool {
    ...
    public var summary: String {
        var out =
            "\(Int(size)) · зерно \(String(format: "%.3f", grain * 100))% · \(particleCount) частиц"
