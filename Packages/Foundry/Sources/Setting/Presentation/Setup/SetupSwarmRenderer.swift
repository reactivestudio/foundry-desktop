...
/// Рендер роя мастера настройки: частицы в линейный буфер (+ световые следы разлёта),
...
/// рой мастера настройки), в одну абстракцию — а преждевременное «DRY» здесь и есть
...
final class SetupSwarmRenderer {

    /// Раскладка должна совпадать с SwarmUniforms в SetupSwarm.metal.
    ...
    private static func makeLibrary(device: MTLDevice) throws -> MTLLibrary {
        ...
        guard let url = Bundle.module.url(forResource: "SetupSwarm", withExtension: "metal") else {
            throw SetupError.resourceMissing(
                "в бандле нет ни default.metallib с ройем, ни SetupSwarm.metal")
        }
        let source: String
        do { source = try String(contentsOf: url, encoding: .utf8) } catch {
            throw SetupError.resourceMissing("не читается SetupSwarm.metal: \(error)")
        }
        do { return try device.makeLibrary(source: source, options: nil) } catch {
            throw SetupError.resourceMissing("не компилируется SetupSwarm.metal: \(error)")
        }
    }
