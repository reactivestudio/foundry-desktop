...
/// Рой мастера настройки во всю рабочую зону окна установки. Живёт всегда (морфинг из
...
/// и не меняет размер.
struct SetupSwarmView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> MTKView {
        ...
        // фон даёт нижний SwiftUI-слой SetupStyle.backdrop. Непрозрачный слой залил бы его.
    ...
    final class Coordinator: NSObject, MTKViewDelegate {
        var parent: SetupSwarmView
        private var renderer: SetupSwarmRenderer?
        ...
        init(_ parent: SetupSwarmView) { self.parent = parent }

        func attach(view: MTKView) {
            guard let device = view.device else { return }
            do {
                renderer = try SetupSwarmRenderer(
                ...
                FeatureLog.swarm.fault(
                    "рой мастера настройки не собрался: \(error.localizedDescription, privacy: .public)")
            }
        ...
        func setBursting(_ isBursting: Bool) {
            ...
            if isBursting {
                burstStart = CACurrentMediaTime() + SetupSwarmView.burstDelay
                burst = 0
        ...
        private func recomputeLayout(view: MTKView, pixelSize: CGSize) {
            ...
            let geometry = SetupSwarmView.self
        ...
        private func advanceBurstTimeline(now: CFTimeInterval, seconds: Float) {
            ...
            if isBursting {
                if shouldReduceMotion {
                    ...
                    let progress = max(0, (now - burstStart) / SetupSwarmView.burstDuration)
        ...
        private func makeUniforms(seconds: Float) -> SetupSwarmRenderer.SwarmUniforms {
            let geometry = SetupSwarmView.self
            var u = SetupSwarmRenderer.SwarmUniforms()
