...
// Рой мастера настройки — порт docs/design/mockups/setup.html (принятый макет,
// PR #34) из GLSL ES 3.0 в MSL. Тот же материал, что Orb/OrbSwarm.metal, но с
// добавленными ручками мастера настройки: aspect (окно не квадратное), center/fit
...
// копия ядра с хореографией мастера настройки. Расхождение чисел с прототипом —
// баг, а не вкус.
//
...
struct SwarmUniforms {
    float  time;
    float  count;
    float2 resolution;      // размер БУФЕРА в пикселях
    float  zoom;
    float  pointSize;       // размер точки в пикселях буфера
    ...
    float  jitter;      // блюр хвоста: боковой сдвиг тусклой копии, px буфера
...
// Фон роя — тот пиксель, что реально лежит под роем: SetupStyle.backdrop #0E0B14
// (linear по гамме 2.2, той же, которой красятся частицы). Число обязано
// совпадать с подложкой: пол хвоста светлее фона превращает след из гаснущего
// хвоста в сплошной штрих ровной яркости (так и было с осветлением до #241E3B,
// оставшимся от снятой завесы).
constant float3 BG_LIN = float3(0.001687, 0.000992, 0.003698);
...
vertex VOut swarmVertex(uint vid [[vertex_id]],
    ...
    if (U.mode == 1 && abs(U.jitter) > 0.001) {
        float2 rd = proj - U.center;
        float2 perp = normalize(float2(-rd.y, rd.x) + float2(1e-5, 0.0));
        proj += perp * (U.jitter * pow(sIdx / float(SEGS), 1.5) * 2.0 / U.resolution.y);
    }
    ...
    if (U.mode == 1) {
        ...
        if (abs(U.jitter) > 0.001) shade *= 0.30 * smoothstep(0.15, 0.65, sN);
    }
    ...
    out.pointSize = U.pointSize * mix(1.0, U.taper, depth) * (1.0 - 0.35 * bFade);
    return out;
}
...
fragment float4 swarmPostFragment(PostOut in [[stage_in]],
    ...
    uint2 base = uint2(in.position.xy) * uint(U.supersample);
    float3 sumC = float3(0.0), sumW = float3(0.0);
    int nC = 0, nW = 0, nB = 0;
    for (int j = 0; j < U.supersample; j++) {
        for (int i = 0; i < U.supersample; i++) {
    ...
    // (SetupStyle.backdrop). Где нет частиц — alpha 0. Где есть — premultiplied-альфа = доля
    ...
    float cov = float(lit) / float(U.supersample * U.supersample);
