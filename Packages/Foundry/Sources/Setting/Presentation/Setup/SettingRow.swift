    ...
    var body: some View {
        HStack(spacing: 16) {
            VStack(alignment: .leading, spacing: 2) {
                Text(name).font(.system(size: 12, weight: .semibold)).foregroundStyle(SetupStyle.Text.primary)
                ...
                Text(description).font(.system(size: 11)).foregroundStyle(SetupStyle.Text.tertiary)
                    .lineLimit(1)
            }
            Spacer(minLength: 12)
            trailing
