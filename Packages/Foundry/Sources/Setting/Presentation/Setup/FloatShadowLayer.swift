                ...
                ZStack {
                    ForEach(casters.indices, id: \.self) { i in
                        ...
                        RoundedRectangle(cornerRadius: casters[i].radius, style: .continuous)
                            .fill(SetupShadow.color)
                            .frame(
                                width: rect.width + 2 * SetupShadow.spread,
                                height: rect.height + 2 * SetupShadow.spread
                            )
                            .position(x: rect.midX, y: rect.midY)
                    }
                }
                .compositingGroup()
                .blur(radius: SetupShadow.blur)
                .opacity(SetupShadow.opacity)
            }
        }
    }
}
