...
/**
 ...
 каким сделать окно — забота корня композиции (`SetupGateView` в `Bootstrap`).
    ...
    private var footer: some View {
        ...
        ZStack {
            SetupDots(
                ...
                onTap: { index in
                    if let target = SetupModel.Screen(rawValue: index) { model.go(to: target) }
                }
            )
            .frame(maxWidth: .infinity)
            ...
            .offset(y: SetupDots.hitHeight / 2 - SetupDots.dotCenterFromBottom)
        }
        .frame(height: SetupDots.hitHeight)
        .opacity(model.isBursting ? 0 : 1)
        .animation(SetupStyle.easeReal(0.15), value: model.isBursting)
    }
}
