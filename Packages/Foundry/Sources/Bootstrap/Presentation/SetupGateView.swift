...
/**
 ...
 (`WindowConfigurator`). Потому и живёт в корне композиции: мастер (`SetupView` из
 ...
 */
struct SetupGateView<MainContent: View>: View {
    ...
    /// хранилище.
    private var didFinishSetup: Bool {
    ...
    var body: some View {
        ZStack {
            ...
            // идёт мастер настройки, он ПОЛНОСТЬЮ инертен: TextEditor промпта консоли —
            ...
            mainContent
                .disabled(!didFinishSetup)
                .allowsHitTesting(didFinishSetup)
            ...
            if !didFinishSetup {
                ...
                // (RunConsoleView, SetupStyle.bg #05030D) плоской «плашкой». Здесь
        ...
        // Второй аргумент — не дубль первого: `isSetupActive` это состояние ЭТОГО
        ...
        .background(
            WindowConfigurator(
                isSetupActive: !didFinishSetup,
                isSetupActiveNow: { !preferenceStore.setup.isFinished }
            )
        )
    }
}
