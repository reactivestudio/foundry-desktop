...
@MainActor
final class ManualSetupScheduler: SetupScheduler {
...
@MainActor
@Suite("Модель мастера настройки")
struct SetupModelTests {
    ...
    func initialState() {
        let model = SetupModel(scheduler: ManualSetupScheduler())
    ...
    func grantedPermissionsAdvanceToReady() {
        let scheduler = ManualSetupScheduler()
        let model = SetupModel(scheduler: scheduler)
