...
struct OrbSwarmView: View {
    ...
    static let minimumUsefulSize: CGFloat = 64
    ...
    /// непрозрачный слой роя проступит на фоне квадратом — фон ленты и фон роя
    /// задаются одним значением, не двумя.
    static let canvas = Color(red: 5 / 255, green: 6 / 255, blue: 12 / 255)
...
private struct OrbSwarmLayer: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, MTKViewDelegate {
        ...
        func configure(view: MTKView, size: CGFloat, preset: OrbSwarmConfig.Preset) {
            ...
            if let renderer {
                ...
                do {
                    ...
                    // Но и терять сбой в релизе (где assert — no-op) нельзя:
                    // пишем fault в системный лог.
                    FeatureLog.swarm.fault("рой не собрался: \(error.localizedDescription, privacy: .public)")
                    return
                }
            }
            config = cfg
        ...
        private func applyFrameRate(view: MTKView, preset: OrbSwarmConfig.Preset) {
            let displayHz = view.displayRefreshHz
            ...
            assert(
                !OrbSwarmConfig.stutters(preset: preset, displayHz: displayHz),
        ...
        func setAnimating(_ on: Bool, view: MTKView) {
            ...
            if on {
                ...
                startTime = CACurrentMediaTime() - CFTimeInterval(frozenTime)
            }
            animating = on
            view.isPaused = !on
        ...
        func draw(in view: MTKView) {
            ...
            let time = animating ? Float(CACurrentMediaTime() - startTime) : frozenTime
            if animating { frozenTime = time }
            renderer.encode(into: commandBuffer, output: drawable.texture, time: time)
