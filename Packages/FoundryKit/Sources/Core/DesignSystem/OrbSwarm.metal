...
struct OrbUniforms {
    float time;
    float count;
    float2 resolution;   // размер БУФЕРА в пикселях (не экрана и не точек)
    float zoom;
...
struct ResolveUniforms {
    float time;
    int   supersample;   // во сколько раз буфер крупнее выхода
};
...
vertex VOut orbVertex(uint vid [[vertex_id]],
    ...
    out.pointSize = U.pointSize * mix(1.0, U.taper, depth);
    return out;
}
...
fragment float4 postFragment(PostOut in [[stage_in]],
    ...
    if (lit == 0) {
        ...
        c = (avg * float(lit) + BG * float(nB)) / float(U.supersample * U.supersample);
    }
